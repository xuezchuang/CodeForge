...
fn code_navigation_response_guidance() -> &'static str {
    concat!(
        ...
        "Only create clickable Markdown code links when the target includes one concrete line number, for example [Foo.cpp:123](src/module/Foo.cpp:123). ",
        "Do not create file-only Markdown links such as [Foo.cpp](src/module/Foo.cpp); if only the file is known, cite it as plain inline code or inspect/search for the relevant line first. ",
        "Do not fabricate a line number when the exact line has not been verified. ",
...
fn tool_error_result(error: &str) -> Value {
    json!({
        ...
        "output": null,
        "error": error,
        "recoveryHint": "The tool failed. If a path was not found, first re-list the parent directory with list_dir (do not invent path segments), then retry search_file/search_content from a confirmed valid parent or absolute root. Read tools can inspect local paths outside the active workspace; write and edit tools remain workspace-bound."
    })
}
